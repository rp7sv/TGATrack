...
class VisionTransformerP(VisionTransformer):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dim=768, depth=12,
        ...
        '''prompt parameters'''
        if self.prompt_type in ['tgatrack_shaw', 'tgatrack_deep']:
            prompt_blocks = []
            block_nums = depth if self.prompt_type == 'tgatrack_deep' else 1
    ...
    def forward_features(self, z, x, mask_z=None, mask_x=None,
        ...
        if self.prompt_type in ['tgatrack_shaw', 'tgatrack_deep']:
            ...
            z_feat = self.prompt_blocks[0](z_feat) # 两种模态的template融合
            x_feat = self.prompt_blocks[0](x_feat) # 两种模态的search融合
            z_dte = feature2token(z_feat)
            x_dte = feature2token(x_feat)
            z_prompted, x_prompted = z_dte, x_dte

            # T模态通过融合后续以prompt的形态与rgb交互
            x = x + x_dte 
            z = z + z_dte
        else:
            x = x + x_dte
        ...
        for i, blk in enumerate(self.blocks):
            ...
            '''
            if i >= 1:
                if self.prompt_type in ['tgatrack_deep']:
                    x_ori = x
                    # prompt
