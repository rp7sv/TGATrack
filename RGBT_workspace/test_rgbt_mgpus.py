...
from lib.test.tracker.tgatrack import TGATrack
import lib.test.parameter.tgatrack as rgbt_prompt_params
...
def run_sequence(seq_name, seq_home, dataset_name, yaml_name, num_gpu=1, epoch=300, debug=0, script_name='prompt'):
    ...
    if script_name == 'tgatrack':
        params = rgbt_prompt_params.parameters(yaml_name, epoch)
        mmtrack = TGATrack(params)  # "GTOT" # dataset_name
        tracker = TGATrack_RGBT(tracker=mmtrack)
    ...
    for frame_idx, (rgb_path, T_path) in enumerate(zip(RGB_img_list, T_img_list)):
        ...
        if frame_idx == 0:
            ...
            # track
            image = get_x_frame(rgb_path, T_path, dtype=getattr(params.cfg.DATA,'XTYPE','rgbrgb'))
            region, confidence = tracker.track(image, seq_path)  # xywh
...
class TGATrack_RGBT(object):
    ...
    def track(self, img_RGB, seq_path):
        '''TRACK'''
        outputs = self.tracker.track(img_RGB, seq_path)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--threads', default=44, type=int, help='Number of threads')
